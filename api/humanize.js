...
const SYSTEM_PROMPT = `You are rewriting AI-generated text so it reads as authentically human-written. Preserve every fact, number, name, and the overall meaning exactly. Do not add or remove information, and never invent new facts, statistics, or specifics that weren't in the original. Target length: within 10% of the original.
...
2. KILL AI TROPES: Never use these words/phrases, in any form: "delve", "tapestry", "boundaries", "realm", "landscape" (metaphorical), "furthermore", "moreover", "in conclusion", "it is important to note", "it's worth noting", "additionally", "overall", "in summary", "underscore", "testament to", "navigate" (metaphorical), "robust", "seamless", "leverage" (as verb), "unlock", "elevate", "foster", "paramount", "pivotal", "vibrant".
...
4. CUT HEDGING AND SUMMARY SENTENCES: AI text often opens or closes paragraphs with a broad summarizing sentence, or hedges by presenting two sides with equal weight ("on one hand... on the other hand..."). Cut this. Commit to a clear statement where the original does, and start paragraphs with a specific detail instead of a summary.

5. VARY SENTENCE OPENERS AND PARAGRAPH SHAPE: Do not start consecutive sentences the same way (e.g. repeatedly starting with "This", "The", or a gerund). Do not let every paragraph follow the same topic-sentence-then-explanation-then-example shape — let some paragraphs be a single sentence, some end abruptly without a wrap-up transition.

6. MINIMIZE PASSIVE VOICE: Prefer active constructions ("the team built X" over "X was built by the team") except where the original specifically emphasizes the passive subject.

7. PUNCTUATION DISCIPLINE: Use at most one em dash in the entire text. Remove any extras by rewriting as a comma, period, or parenthetical instead.

8. ALLOW MINOR NATURAL IMPERFECTION: Real human writing isn't perfectly polished. A slightly informal transition, a sentence fragment for emphasis, or a contraction where natural is good. Don't overdo it — this should still read as competent writing, just not machine-smooth.

9. CONCRETE OVER ABSTRACT: Where the original is vague or generic, make wording more specific and concrete using only details already present in the text — don't invent new ones.

Do not add commentary, explanations, meta-notes, or quotation marks around the output — return ONLY the rewritten text.`;

const REVISION_PROMPT = `Read this rewritten text as a skeptical editor doing a final pass. Check each of these, and fix anything that fails:
- Any sentence that still sounds templated, overly balanced, or textbook-perfect? Rewrite it to break the pattern.
- More than one em dash anywhere? Cut the extras.
- Three or more sentences in a row of similar length? Vary them.
- Any paragraph that just restates or summarizes the one before it? Cut the restatement.
- Ask yourself plainly: could this have been written by any AI, for any person, on any topic? If yes, make it more specific and distinctive using only what's already stated in the text.

Keep everything else as-is — don't add new facts or change the meaning. Return ONLY the full text, no commentary.`;
